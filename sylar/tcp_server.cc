...
namespace mysylar
{
static mysylar::Logger::ptr g_logger = SYLAR_LOG_NAME("system/tcp_server");

static mysylar::ConfigVar<uint64_t>::ptr g_tcp_server_read_timeout = mysylar::Config::Lookup(
...
TcpServer::TcpServer(IOManager* worker, IOManager* accept_worker)
    : m_worker(worker), m_acceptWorker(accept_worker),
      m_recvTimeout(g_tcp_server_read_timeout->getValue()), m_name("mysylar/1.0.0"), m_isStop(true)
{
}
TcpServer::~TcpServer()
{
...
void TcpServer::startAccept(Socket::ptr sock)
{
    while (!m_isStop)
    {
        ...
        {
            SYLAR_LOG_ERROR(g_logger)
                << "accept fail errno=" << errno << " errstr=" << strerror(errno);
        }
    }
}
