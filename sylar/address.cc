...
{
    // addrinfo 是 POSIX 网络编程里用于“地址解析”的核心结构体，配合 getaddrinfo() 使用。
    ...
    if (!host.empty() && host[0] == '[')
    {
        ...
        // 在一段内存的前 n 个字节中，查找第一个等于c的字节
        // s起始地址，c要查找的字节，n搜索长度（字节数）
    ...
    // 如果此时node为空，就说明不是ipv6格式，可能是"www.baidu.com:80"或"192.168.1.1:8080"这种ipv4
    if (node.empty())
    {
